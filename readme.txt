...
WARNING: All Yamaha profiles have a strange bug, they react on 10k+ sounds (especially when plugged in an amp) and produce an awful noise. They can react on background noise of an instrument, for example. I don't know what causes it, and I try to find some way to fix it.
...
6. Ampeg BA-115 - a small 100W combo amp
7. Yerasov 7000 Volt - merciless bag of evil sand. A very angry distortion pedal that sounds close to Digitech Death Metal
8. Caline CP-82 Foe Hammer - bitch

ATTENTION:
am3data profiles are not supported by old Tank firmware, in order to use those profiles you have to go to cuvave site, download the firmware app, download the fresh firmware (TANK Bv2) and install it.
MORE ATTENTION: all preset data will be erased from banks during the upgrade, so be sure to make a backup
